    ...
    return `
      <li class="result">
          ...
          <div class="result-info">
              <h3><a href="${data.website ? data.website : '#'}">${data.name}</a> welcomes ${_.map(data.services, s => `<span class="result-service">${s.toLowerCase().replace('esl', 'ESL')}</span>`).join(', ')} help.</h3>
